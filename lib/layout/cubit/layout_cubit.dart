...
Future<void> getMyJoinedPlansData() async {
  ...
  if (userID?.isNotEmpty == true) { // <-- use the null-aware operator to check if userID is not empty or null
    await FirebaseFirestore.instance
        .collection('users')
        .doc(userID)
        .collection('own_plans')
      ...
      for (var planItem in items.docs) {
        if (joinedPlansID.contains(planItem.id)) {
    ...
    emit(GetJoinedPlansErrorState()); // <-- handle the error if userID is empty or null
  }
}
